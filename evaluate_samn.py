"""
SAMN-EPI Evaluation Script — Cross-Cell-Line Testing

Loads a trained SAMN-EPI checkpoint and evaluates on specified test cell lines.
...
def save_curves(labels, preds, save_dir, prefix=""):
    ...
    ax.plot(fpr, tpr, "b-", linewidth=2,
            label=f"SAMN-EPI (AUROC = {auc_val:.4f})")
    ax.plot([0, 1], [0, 1], "k--", alpha=0.5)
    ax.set_xlabel("False Positive Rate", fontsize=12)
    ax.set_ylabel("True Positive Rate", fontsize=12)
    ax.set_title(f"{prefix}ROC Curve — SAMN-EPI", fontsize=14)
    ...
    ax.set_title(f"{prefix}Precision-Recall Curve — SAMN-EPI", fontsize=14)
    ax.legend(fontsize=11)
    ax.grid(True, alpha=0.3)
    fig.tight_layout()
...
def main():
    ...
    if not os.path.exists(encoder_path):
        ...
        print("Run train_samn.py first to fit and save the encoder.")
        return
    ...
    if ckpt_path is None:
        ckpt_path = os.path.join(save_dir, "samn_model_best.pth")
        if not os.path.exists(ckpt_path):
            ckpt_path = os.path.join(save_dir, "samn_model_final.pth")
